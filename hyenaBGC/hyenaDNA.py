...
Flash Attention support added for improved memory efficiency and speed.
"""
...
class FlashSelfAttention(nn.Module):
    """Flash Attention implementation using flash-attn library.
    
    This provides O(N) memory complexity instead of O(N^2) for standard attention,
    making it much more efficient for long sequences.
    ...
    def __init__(self, causal=False, softmax_scale=None, attention_dropout=0.0):
        super().__init__()
        assert FLASH_ATTENTION_AVAILABLE, "Flash Attention is not installed!"
        self.causal = causal
    ...
    def forward(self, qkv, causal=None, key_padding_mask=None):
        ...
        ---------
            ...
            key_padding_mask: Not used in Flash Attention (for API compatibility)
        """
        causal = self.causal if causal is None else causal
        
        # Flash attention expects (batch, seqlen, 3, nheads, headdim)
        # qkv is already in this format
...
class SelfAttention(nn.Module):
    ...
    Automatically uses Flash Attention when available for better performance.
    ...
    """
    def __init__(self, causal=False, softmax_scale=None, attention_dropout=0.0, use_flash_attn=True):
        super().__init__()
        self.causal = causal
        self.softmax_scale = softmax_scale
        self.dropout_p = attention_dropout
        ...
        if self.use_flash_attn:
            print("Using Flash Attention for SelfAttention")
...
class MHA(nn.Module):
    ...
    Supports Flash Attention for improved efficiency with long sequences.
    """

    def __init__(self, embed_dim, num_heads, bias=True, dropout=0.0,
                 softmax_scale=None, causal=False, layer_idx=None, dwconv=False,
                 return_residual=False, use_flash_attn=True, device=None, dtype=None) -> None:
        """
        ...
        self.inner_attn = inner_attn_cls(causal=causal, softmax_scale=softmax_scale,
                                         attention_dropout=dropout, use_flash_attn=use_flash_attn)
...
def create_mixer_cls(layer=None,
                     ...
                     use_flash_attn=True, device=None, dtype=None):
    """Create mixer class (Hyena or Attention).
    
    Args:
        layer: Layer config for HyenaOperator
        attn_layer_idx: Indices of layers that should use attention instead of Hyena
...
def create_block(d_model, d_inner=None,
                 ...
                 use_flash_attn=True, layer_idx=None,
                 device=None, dtype=None):
    """Create a transformer block with Hyena or Attention mixer.
    
    Args:
        d_model: Model dimension
        d_inner: Inner dimension for MLP
        layer: Layer config for HyenaOperator
        attn_layer_idx: Indices of layers using attention
        attn_cfg: Attention configuration
        layer_norm_epsilon: LayerNorm epsilon
        resid_dropout1: First residual dropout
        resid_dropout2: Second residual dropout
...
class LMBackbone(nn.Module):
    ...
    def __init__(self, d_model: int, n_layer: int, d_inner: int, vocab_size: int,
                 ...
                 residual_in_fp32=False, use_flash_attn=True,
        ...
        self.layers = nn.ModuleList([create_block(
            ...
            use_flash_attn=use_flash_attn, layer_idx=i,
            **factory_kwargs,
...
class HyenaDNACausalLM(nn.Module):
    """
    HyenaDNA model with a causal language modeling head for DNA sequence generation.
    
    This model extends HyenaDNAModel with a linear output layer for next-token prediction,
    enabling autoregressive generation of DNA sequences.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize HyenaDNA Causal LM.
        
        Args:
            config: HyenaDNAConfig object (if provided, overrides individual args)
            d_model: Model dimension
            n_layer: Number of layers
            d_inner: FFN hidden dimension
            vocab_size: Size of vocabulary
            l_max: Maximum sequence length
            order: Hyena operator order
            filter_order: Filter order for Hyena
            resid_dropout: Residual dropout rate
            embed_dropout: Embedding dropout rate
            layer_norm_epsilon: LayerNorm epsilon
            residual_in_fp32: Whether to use FP32 for residuals
        """
        super().__init__()
        ...
        if config is not None:
            ...
            d_inner = getattr(config, 'd_inner', None) or 4 * d_model
            vocab_size = config.vocab_size
            l_max = config.l_max
            order = getattr(config, 'order', 2)
            filter_order = getattr(config, 'filter_order', 64)
            resid_dropout = getattr(config, 'resid_dropout', 0.0)
        ...
        self.backbone = LMBackbone(
            ...
            attn_layer_idx=None,
            attn_cfg=None,
            max_position_embeddings=0,  # Hyena doesn't use position embeddings
    ...
    def forward(self, input_ids, labels=None, attention_mask=None, return_dict=True, **kwargs):
        ...
        Args:
            ...
            attention_mask: Attention mask (not used by Hyena, kept for compatibility)
    ...
    def generate(
        ...
        **kwargs
        ...
        """
        Autoregressive generation of DNA sequences.
        
        Args:
            input_ids: Optional prompt tokens (batch_size, prompt_len)
        ...
        if input_ids is None:
            input_ids = torch.tensor([[2]], device=device)  # Start with BOS token
        else:
        ...
        for _ in range(max_length - input_ids.shape[1]):
            ...
            if generated.shape[1] > self.backbone.layers[0].mixer.l_max:
                context = generated[:, -self.backbone.layers[0].mixer.l_max:]
            else:
                context = generated
            
            # Forward pass
            outputs = self.forward(context, return_dict=True)
    ...
    def get_num_params(self, non_embedding: bool = True):
        ...
        if non_embedding:
            n_params -= self.backbone.embeddings.word_embeddings.weight.numel()
        return n_params
    
    @classmethod
    def from_pretrained(cls, pretrained_model_path, device='cpu', config=None, **kwargs):
        """
        Load a pretrained HyenaDNA model.
        
        Args:
            pretrained_model_path: Path to model directory or HuggingFace model name
            device: Device to load model on
            config: Optional config override
            
        Returns:
            model: Loaded HyenaDNACausalLM model
        """
        import os
        ...
        if isinstance(config, dict):
            # Convert dict to config-like object
            class ConfigDict:
                pass
            cfg = ConfigDict()
            for k, v in config.items():
                setattr(cfg, k, v)
            model = cls(config=cfg, **kwargs)
        else:
        ...
        if not os.path.exists(weights_path):
            weights_path = os.path.join(model_path, 'weights.ckpt')
